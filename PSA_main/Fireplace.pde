...
class Fireplace {
  ...
  void check(float tempBucketX, float tempBucketY, boolean initBucketFill, float tempShovelX, float tempShovelY, float tempMouseX, float tempMouseY, boolean initOverBucket, boolean initOverShovel) {
    ...
    if (drown && stir && !isBurning && drown2 && !initOverBucket && !initOverShovel && (tempMouseY >=yPosition-100) && (tempMouseY <=yPosition+300)&& (tempMouseX >= (xPosition-50)) && (tempMouseX<= xPosition+50)) {  //if the person is over the fire and all other steps have been done, let boolean feel become true.
      feel = true;  //step feel completed
    }
  }
}
